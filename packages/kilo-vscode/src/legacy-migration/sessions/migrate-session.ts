...
export async function migrateSession(id: string, context: vscode.ExtensionContext, client: KiloClient) {
  ...
  // for (const msg of payload.messages) await client.kilocode.sessionImport.message(msg, { throwOnError: true })
  // for (const part of payload.parts) await client.kilocode.sessionImport.part(part, { throwOnError: true })
